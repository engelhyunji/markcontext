<%@ page language="java" contentType="text/html; charset=utf-8" pageEncoding="utf-8"%>
<%@include file="MaFpsCommon.jsp"%>
<%@include file="MaFunction.jsp"%>

<%--
================================================================================
파일명(추정): MaFpsTail_Noax.jsp
역할:
  1) RT 원본 데이터와 PDF 경로를 요청 파라미터로 받습니다.
  2) MarkAny Java 라이브러리를 호출하여 FPS/RD 데몬에서 2D 바코드용 메타데이터를 생성합니다.
  3) 반환된 메타데이터에 Viewer 출력 정책을 붙여 .matmprp 파일로 저장합니다.
  4) 저장 파일의 다운로드 URL과 JSESSIONID를 JavaScript 변수로 내려줍니다.
  5) MaXHRControl.js와 MaVerCheck.js를 이용하여 Broker 설치 확인 및 Viewer EXE 실행을 요청합니다.

중요 연결:
  MaFpsTail_Noax.jsp
    -> JavaScript 변수 생성(vstrSDownURL, vstrSCookie, chkFileArray, executeBinaryArray)
    -> MaVerCheck.js: showPopup(), maFun_InstallCheck(), maFun_execute()
    -> MaXHRControl.js: maBrokerInit(), craeteBoekerCommand(), maBroker_wss()
    -> MaEPSBroker.exe
    -> ePageSaferRT.exe
    -> Mafndown.jsp?fn=<시간값>
    -> sessionId + 시간값 + ".matmprp" 다운로드

주의:
  이 주석본은 동작을 변경하지 않고 설명 주석만 추가한 분석용 파일입니다.
================================================================================
--%>

<%
    /*
     * [1단계] 발급/리포팅 JSP가 전달한 RT 입력값을 받습니다.
     *
     * RtData       : OZ/RP/RD/UB 등 리포팅 엔진이 만든 원본 데이터 파일 경로
     * PdfFilePath  : Viewer가 표시하거나 바코드를 합성할 PDF 파일 경로
     * PdfDataType  : 문서 방향·PDF 처리 유형을 구분하는 값
     * PrintOptions : 미리보기/자동인쇄/기본프린터 등 Viewer 동작 옵션
     */
	//Get RT Jsp ( PDF, rtdata, doc Type)
	String strRtData = request.getParameter("RtData");
	String strPdfFilePath = request.getParameter("PdfFilePath");
	String strPdfDataType = request.getParameter("PdfDataType");
	int iPrintOptions = Integer.parseInt(request.getParameter("PrintOptions"));	
	
	
	// 20241119_smlee_Start: index.jsp에서 바코드 사이즈 입력 시 RD 오류 해결
    /*
     * 요청을 보낸 이전 JSP의 URL을 확인합니다.
     * Sample_RD.jsp이면 RD 전용 데몬 포트(18300)와 회사 식별값을 사용합니다.
     *
     * 주의: 현재 원본은 referer의 null 여부를 검사하기 전에 아래 sampleName을 만듭니다.
     * Referer 헤더가 없으면 NullPointerException이 발생할 수 있으므로 운영 코드에서는
     * referer != null 확인 후 sampleName을 계산하는 편이 안전합니다.
     */
	String referer = request.getHeader("Referer");
	String sampleName = referer.replaceAll(".*/", "").split("\\?")[0];
	//System.out.println("MaFpsTail_Noax.jsp_referer: " + referer);
	//System.out.println("MaFpsTail_Noax.jsp_sampleName: " + sampleName);

	
	if (referer != null && sampleName.equals("Sample_RD.jsp")) {
		iMAServerPort 	= 18300;
		strReportCompany = "RD##";
    }
	
	else if(referer == null) {
        System.out.println("Failed to get the referer");
    }
	
	//System.out.println("MaFpsTail_Noax.jsp_iMAServerPort: " + iMAServerPort);
	//System.out.println("MaFpsTail_Noax.jsp_strReportCompany: " + strReportCompany);
	// 20241119_smlee_End: index.jsp에서 바코드 사이즈 입력 시 RD 오류 해결
	
	
    /*
     * [2단계] Broker 실행 방식을 결정합니다.
     * BrokerOptions가 전달되면 MaFpsCommon.jsp의 iQuickSet 값을 덮어씁니다.
     *
     * 대표 의미:
     *   iQuickSet == 2 : MaEPSBroker를 이용한 XHR/WebSocket 설치 확인 및 EXE 실행
     *   그 외          : 커스텀 URL 스킴/세션 체크 방식
     */
	// cjkang 20190403 추가 민원용
	if(request.getParameter("BrokerOptions") != null){
	iQuickSet = Integer.parseInt(request.getParameter("BrokerOptions"));
	}
	
    /*
     * URL 파라미터로 인코딩되어 들어온 실제 서버 파일 경로를 복원합니다.
     * 복원된 값은 사용자 PC 경로가 아니라 WAS가 접근 가능한 서버 측 경로입니다.
     */
	strRtData = maEncUtil.strUrlRdDecode( strRtData );
	strPdfFilePath = maEncUtil.strUrlRdDecode( strPdfFilePath );
	// hhw 0514
	// strMakeMdm2InRptData 호출 전 파일 존재 여부 확인
    /*
     * FPS 데몬 호출 전에 RT 원본과 PDF가 실제로 존재하는지 선검사합니다.
     * 파일이 없으면 이후 데몬 오류(9001/90001)가 나기 전에 JSP 처리를 종료합니다.
     */
	File rtDataFile = new File(strRtData);
	if (!rtDataFile.isFile()) {
		System.err.println("[ERROR] RtData file does not exist: " + strRtData);
		return;
	}

	File pdfFile = new File(strPdfFilePath);
	if (!pdfFile.isFile()) {
		System.err.println("[ERROR] PDF file does not exist: " + strPdfFilePath);
		return;
	}
	//hhw 0514 end
    /*
     * Viewer가 나중에 같은 문서를 내려받으려면 HTTP 세션이 필요합니다.
     * 기존 세션이 없으면 새 세션을 생성합니다.
     */
	HttpSession session1 = request.getSession(false);
	if (session1 == null) {
		session1 = request.getSession(true);
	}
	// ksk
	String strOrgFilePath = str2DGenUrl + strRtData + "&PdfType=" + strPdfDataType;

    /*
     * [3단계] RD 전용 바코드 설정을 받습니다.
     * RD는 일반 OZ/RP 계열과 호출 함수 및 설정 방식이 다르므로 별도 분기합니다.
     */
	//RD인경우 Invoker에서 파라미터로 받음
	if ( iMAServerPort == 18300 && iRdConfigDataUse == 1){
		try{
			iCellBlockCount							= Integer.parseInt( request.getParameter("Barcodecount") );
			iCellBlockRow							= Integer.parseInt( request.getParameter("BarcodeRow") );
			strConfigFilePath 						= request.getParameter("ConfigFilePath"); 
			strConfingEncodeData					= request.getParameter("ConfingEncodeData");
		}catch(Exception e){

			out.println("바코드를 생성하기 위한 파라미터가 잘 못 설정되었습니다.");
		}
	}

	
	// 내부 테스트용!! 20241121_hwanghw_Start: 특정 샘플만 바코드 크기 설정 가능
	if (referer != null && sampleName.equals("Sample_OZ_REX.jsp")) {
		iCellBlockCount =Integer.parseInt(request.getParameter("Barcodecount"));
		iCellBlockRow = Integer.parseInt(request.getParameter("BarcodeRow"));
		//iCellBlockCount = 19.6;
		//iCellBlockRow = 2.2;
		//System.out.println("MaFpsTail_Noax.jsp_iCellBlockCount: " + iCellBlockCount);
		//System.out.println("MaFpsTail_Noax.jsp_iCellBlockRow: " + iCellBlockRow);
	}
	// 20241121_hwanghw_End: 특정 샘플만 샘플만 바코드 크기 설정 가능



    /*
     * [4단계] 가로 문서용 2D 바코드 배치 설정을 구성합니다.
     * 설정 문자열은 데몬이 해석하는 구분자(^) 기반 제품 전용 설정값이며,
     * EUC-KR 바이트로 만든 뒤 Base64로 인코딩해 Java 라이브러리에 전달합니다.
     */
	//가로문서이고 RD리포팅이 아닐경우 strMAConfigData값 세팅
	if(strPdfDataType.equals("1") && iMAServerPort != 18300){ 
		strMAConfigData	 						= "31^177^3^177^528^264^인터넷발급^사본^4^280^70^0^0^0^"; // 가로		
		//strMAConfigData	 						= "35^175^7^175^528^264^인터넷발급^사본^4^280^70^0^0^3^"; 
		//strMAConfigData	 						= "35^245^7^245^528^264^인터넷발급^사본^4^280^70^0^0^3^";       				// 세로문서
		strConfingEncodeData 					= MaBase64Utils.base64Encode(strMAConfigData.getBytes("euc-kr"));
	}
	
    /*
     * [5단계] Viewer 표시와 인쇄 동작 정책을 설정합니다.
     *
     * DEFAULTPRT    : 기본 프린터 사용
     * AUTOPRINT     : Viewer 로드 후 자동 인쇄
     * HIDEPRTBUTTON : Viewer의 인쇄 버튼 숨김
     * HIDEFRAME     : Viewer 화면 없이 바로 인쇄
     */
	/* 프린터	선택옵션 */
	// iPrintOptions - 0:	default
	//iPrintOptions = 10;
	switch (iPrintOptions) {
		case 1 : //view-manualprint-defaultprinter
			//마크애니 뷰어로	미리보기가 가능하며, 인쇄버튼을	누르면 기본프린터로	인쇄되는 옵션
			DEFAULTPRT = "1";
			break;
		case 2 : //view-autoprint-printerselect
			//마크애니 뷰어로	미리보기 되자마자, 프린터선택창이	바로 뜨는	옵션
			AUTOPRINT = "1";
			break;
		case 3 : //view-autoprint-defaultprinter
			//마크애니 뷰어로	미리보기 되자마자, 기본프린터로	자동 인쇄되는	옵션
			AUTOPRINT = "1";
			DEFAULTPRT = "1";
			break;
		case 4 : //view
			//마크애니 뷰어로	미리보기만 가능함	(인쇄버튼	없음)
			HIDEPRTBUTTON = "1";
			break;

		case 10 : //notView_autoPrint_printerselect
			//마크애니 뷰어가	뜨지않고,	프린터선택창이 바로	뜨는 옵션
			HIDEFRAME = "1";
			AUTOPRINT = "1";
			break;
		case 11 : //notView_autoPrint_defaultPrinter
			//마크애니 뷰어가	뜨지않고,	기본프린터로 바로	출력되는 옵션
			HIDEFRAME = "1";
			AUTOPRINT = "1";
			DEFAULTPRT = "1";
			break;
	}
	
    /*
     * iUsePDF 값에 따라 데몬에 PDF 경로를 직접 넘길지 결정합니다.
     * iUsePDF == 1이면 메타 생성 단계에서 PDF 경로 대신 "NONE"을 전달하고,
     * Viewer 실행 파라미터에는 PDF URL과 메타 URL을 함께 전달합니다.
     */
	String strPdfParam = strPdfFilePath;
	if (iUsePDF == 1)
		strPdfParam = "NONE";

	String strMetaData = "";
	String strRetCode = "";
	String strRetData = "";
	int iRetCode = 0;
	
	// 생성 시간 선언
	java.text.SimpleDateFormat formatter = new java.text.SimpleDateFormat("yyyyMMddHHmmssSSS");
	String today = formatter.format(new java.util.Date());
	
	
    /*
     * [6단계] MarkAny Java JAR를 통해 FPS/RD 데몬을 호출합니다.
     *
     * RD(18300):
     *   MaMakeCode.strInsert2DCodeDataFileNonActive(...)
     *
     * OZ/RP/기타:
     *   RT 원본 파일을 byte[]로 읽고
     *   MaFpsMake2DCode.iGen2DCodeNonActive(...)
     *
     * 이 함수 내부에서 strMAServerIP:iMAServerPort로 TCP 소켓 통신하며,
     * 데몬이 전자서명/2D 바코드 데이터/메타데이터를 생성하여 문자열로 반환합니다.
     */
	//RD or other Reporting Create 2D bacode
	// ksk
        iCellBlockCount = 295;
	iCellBlockRow = 100;
	if (iUsePDF < 2) {
		if( iMAServerPort == 18300 ){
			MaMakeCode clMaMakeCode = new MaMakeCode();
			strMetaData =  clMaMakeCode.strInsert2DCodeDataFileNonActive(
											strMAServerIP,
											iMAServerPort,
											strRtData, 
											"0", 
											strConfigFilePath, 
											iCellBlockCount, 
											iCellBlockRow,
											"", 
											strConfingEncodeData,
											strPdfFilePath, 
											strPdfDataType,
											strMobileCodeData.getBytes("utf-8"),
											"",
											"",
											"",
											"",
											"" );
											
			
            /*
             * RD 함수의 반환 규격:
             * 앞 5자리 = 결과 코드, 나머지 = 메타데이터 본문
             */
			strRetCode = strMetaData.substring(0, 5);
			strRetData = strMetaData.substring(5);
			iRetCode = Integer.parseInt(strRetCode);
			
		}else if( iMAServerPort != 18300 ){
			
				File FileRtData = new File(strRtData);
				byte byteRtData[] = new byte[(int)FileRtData.length()];

				if (FileRtData.isFile()) {
					try {
						BufferedInputStream fin = new BufferedInputStream(new FileInputStream(FileRtData));
						fin.read(byteRtData);
						fin.close();
					} catch (Exception e) {
						System.out.println("markany_RtData File Input Error (not find) _for Windows");
					}
				}
				
				MaFpsMake2DCode clMaFpsMake2DCode = new MaFpsMake2DCode();
				//cjkang 20200424 trycatch add
				try{
				strMetaData = clMaFpsMake2DCode.iGen2DCodeNonActive(
												strMAServerIP
												, iMAServerPort
												, strConfingEncodeData 
												, iCellBlockCount
												, iCellBlockRow
												, byteRtData
												, strSignCompany
												, strReportCompany
												, 0
												, ""
												, ""
												, ""
												, strPdfParam
												, strPdfDataType
												, strMobileCodeData.getBytes("utf-8")
												, strMobileCodeConfig
												, "");

                /*
                 * 일반 FPS 함수의 반환 규격:
                 * 앞 4자리 = 결과 코드, 나머지 = 메타데이터 본문
                 */
				strRetCode = strMetaData.substring(0, 4);
				strRetData = strMetaData.substring(4);
				iRetCode = Integer.parseInt(strRetCode);
				}catch(Exception e){
					out.println("[MarkAny Net Library] 2D Barcode Creation Error");
					return;
				}
			
		}
        /*
         * 데몬/JAR 처리 오류를 사용자 응답으로 반환합니다.
         * 1001/10001 : 데몬 연결 실패 가능성
         * 9001/90001 : RT/PDF 경로 또는 원본 데이터 오류 가능성
         * -450       : 바코드 영역 부족
         * 1020       : 생성 시간 초과
         */
		//ePageSAFER Server Err Return
		//TODO 대민서비스로 해당 에러문구가 괜찮을까..?
		if (iRetCode != 0) {
			//out.println("ERROR !! " + iRetCode);
			if (iRetCode == 1001 || iRetCode == 10001)
				out.println("Error code : " + iRetCode + " 마크애니 데몬프로세스를 구동해주세요.");
			else if(iRetCode == 9001 || iRetCode == 90001)
				out.println("Error code : " + iRetCode + " pdf 및 원본 데이터 경로가 정상적이지 않습니다.");
			else if(iRetCode == -450)
				out.println("Error code : " + iRetCode + " 설정되어 있는 2D바코드사이즈가 작습니다.");
			else if(iRetCode == 1020) //cjkang 20200424 add errorcode 1020 
				out.println("Error code : " + iRetCode + " 바코드 생성시간이 초과하였습니다.(대용량출력 - 관리자에게 문의)");
			else
				out.println("Error code : " + iRetCode);
			//out.close();
			return;
		}
	}
	
    /*
     * [7단계] 데몬이 반환한 메타데이터 본문 뒤에 Viewer 정책 정보를 추가할 준비를 합니다.
     * strRetData는 아직 서버 파일로 저장되기 전의 메타 본문입니다.
     */
	int iAMetaDataSize = strRetData.length();
	String strMetaFilePath = "";
	String strAddData = "";
	int iRetBro = 0;
	String browsername = "";
	String browserversion = "";
	int iBrowserVer = 0;
	
	// ============================================ Cookie Set Start ==========================================
	/*
	String strCookie = "";
	Cookie[] cookies = request.getCookies();
	
	if (cookies != null) {
		for (int i = 0; i < cookies.length; i++) {   // 20180716
		//for( int i = cookies.length -1; i >=0; i--){
			for( int cN_i=0; cN_i<registerCookieArr.length; cN_i++){
				if( !cookies[i].getName().equals(registerCookieArr[cN_i]) ){
					continue;
				}
				strCookie += cookies[i].getName() + "="	+ cookies[i].getValue() + ";";
			}
			//break; // 20180716
		}
	}
	strCookie = "Cookie: " + strCookie;
	System.out.println("@@@@@strCookie: "+strCookie);
	*/
	
    /*
     * [8단계] Viewer가 Mafndown.jsp를 호출할 때 같은 세션으로 접근하도록
     * JSESSIONID를 명시적인 HTTP Cookie 문자열로 만듭니다.
     *
     * 이 값은 Base64 인코딩되어 JavaScript의 vstrSCookie로 전달됩니다.
     */
	String sessionId = session.getId(); // 서버가 방금 생성한(또는 유지 중인) 세션 ID를 직접 가져옴
	String strCookie = "Cookie: JSESSIONID=" + sessionId + ";";
	System.out.println("@@@@@strCookie: "+strCookie);
	
/*
	// 1. 초기 설정 및 현재 세션 정보 가져오기
	String strCookie = "";
	String currentSessionId = request.getSession().getId();
	Cookie[] cookies = request.getCookies();
	StringBuilder sb = new StringBuilder();

	// 허용할 쿠키 이름 목록 (기존 registerCookieArr 사용)
	// 예: String[] registerCookieArr = {"USER_ID", "PREFERENCE", ...};

	if (cookies != null) {
		boolean sessionCookieFound = false;

		for (Cookie cookie : cookies) {
			String name = cookie.getName();
			String value = cookie.getValue();

			// A. 현재 서버 세션 ID와 값이 일치하는 경우 (이름 상관없이 무조건 유지)
			if (currentSessionId.equals(value)) {
				if (sb.length() > 0) sb.append("; ");
				sb.append(name).append("=").append(value);
				sessionCookieFound = true;
				continue; // 세션 쿠키로 처리했으므로 다음 쿠키로
			}

			// B. registerCookieArr에 포함된 일반 쿠키인지 확인
			boolean isAllowed = false;
			for (String allowedName : registerCookieArr) {
				if (name.equals(allowedName)) {
					isAllowed = true;
					break;
				}
			}

			// 허용된 쿠키인 경우에만 추가 (단, 위에서 처리된 세션 쿠키와 이름이 겹치면 제외 - 중복 방지)
			if (isAllowed) {
				// 현재 세션 ID를 가진 쿠키와 이름이 같은데 값만 다른 '구형 세션'은 건너뜀
				if (sessionCookieFound && name.equals(getSessNameFromValue(cookies, currentSessionId))) {
					continue; 
				}
				
				if (sb.length() > 0) sb.append("; ");
				sb.append(name).append("=").append(value);
			}
		}

		// 만약 쿠키 목록은 있는데 현재 세션 ID와 일치하는 값이 없었다면 강제 추가
		if (!sessionCookieFound) {
			if (sb.length() > 0) sb.append("; ");
			sb.append("JSESSIONID=").append(currentSessionId);
		}
	} else {
		// 쿠키가 전혀 없는 경우 현재 세션 ID만 추가
		sb.append("JSESSIONID=").append(currentSessionId);
	}
	strCookie = "Cookie: " + sb.toString();
	//out.println("strCookie = " + strCookie);
*/
	
    /*
     * URL/EXE 실행 파라미터의 구분 문자 충돌을 피하려고 Cookie 문자열을 Base64 처리합니다.
     * Base64는 암호화가 아니라 전송 형식 인코딩입니다.
     */
	byte	byteCookieData[] = strCookie.getBytes ("utf-8"); 
	String	strBase64Cookie = MaBase64Utils.base64Encode( byteCookieData );

	// ============================================ Cookie Set E n d ==========================================
	
    /*
     * [9단계] 세션에 기록된 클라이언트 제품 버전과 요구 버전을 비교합니다.
     * 결과 iSessionCheck는 아래에서 기존 세션 방식과 Broker 방식의 실행 분기를 정합니다.
     */
	// ====================================== EP Client Version Check Start ====================================
	int iSessionCheck = 0;
	
	session.setAttribute("productversion", strPVersion);
	String pversion = (String) session.getAttribute("productversion");
	if (pversion != null && pversion.indexOf("MARKANYREPORT") >= 0) {
		String strVersion = replaceAll(pversion, "MARKANYREPORT", "");
		int iCurrent = Integer.parseInt(strPVersion);
		int iSession = Integer.parseInt(strVersion);

		if (iSession >= iCurrent)
			iSessionCheck = 1;
	}
	
	// ====================================== EP Client Version Check End ====================================
	
    /*
     * [10단계] Windows Viewer가 해석할 추가 출력 정책을 #KEY=VALUE 문자열로 만듭니다.
     * 이 값은 데몬 메타데이터(strRetData) 뒤에 이어 붙여 .matmprp 파일로 저장됩니다.
     */
	// ==================================== Window OS Add meta Data Start ==================================
	String info = request.getHeader("User-Agent");
	if (info != null) {
		String browserInfoArr[] = getBrowserInfo(info);
		browsername = browserInfoArr[0];
		browserversion = browserInfoArr[1];
		iBrowserVer = Integer.parseInt(browserversion.trim());

		if ( info.indexOf("Windows") > 0) {
			if (iUsePDF != 2) {
				//MASetICP(m_hImgSf, pIcpInfo.wszText, pIcpInfo.wszFont, pIcpInfo.nICPPattern, pIcpInfo.nICPOpacity, pIcpInfo.nICPDegree, pIcpInfo.nICPTextPercent1, 
				//pIcpInfo.nICPPosXPercent1, pIcpInfo.nICPPosYPercent1, pIcpInfo.nICPTextPercent2, pIcpInfo.nICPPosXPercent2, pIcpInfo.nICPPosYPercent2);
				String strIcpParam = "1^PreView^Wide Latin^7^30^-40^10^0^0^0^0^0^";
				String ICP = MaBase64Utils.base64Encode(strIcpParam.getBytes()); //ICP Setting

				strAddData = "#IMGURL=http://" + strServerName + ":" + String.valueOf(iServerPort) + "/" + strImagePath;
				strAddData += "#META_SIZE="	+ String.valueOf(iAMetaDataSize);
				strAddData += "#CPPARAM=" + "";
				strAddData += "#PRTPROTOCOL=" + request.getScheme(); //
				strAddData += "#PRTIP=" + strServerName;
				strAddData += "#PRTPORT=" + String.valueOf(iServerPort);
				strAddData += "#PRTPARAM=" + strPrintParam;
				strAddData += "#PRTURL=" + strPrintURL;
				strAddData += "#DOCTYPE=1";
				strAddData += "#PRTTYPE=" + PRTTYPE;
				strAddData += "#PRINTCOPIES=" + PRINTCOPIES;
				strAddData += "#PSSTRING=" + PSSTRING;
				strAddData += "#PSSTRING2=" + PSSTRING2;
				strAddData += "#FAQURL=" + FAQURL;
				//strAddData += "#CP2PARAM=" + CP2PARAM;
				strAddData += "#WMPARAM=" + WMPARAM;
				strAddData += "#TITLE=" + TITLE;
				strAddData += "#PRINTERDAT=" + strDataFileName;
				strAddData += "#PRINTERVER=" + PRINTERVER;
				strAddData += "#PRINTERUPDATE=" + PRINTERUPDATE;
				strAddData += "#HIDEPRTBUTTON=" + HIDEPRTBUTTON; //0 or default : show,  1 : hide //미리보기만
				strAddData += "#HIDEFRAME=" + HIDEFRAME; //0 or default : show,  1 : hide //바로출력 미리보기  x
				strAddData += "#DEFAULTPRT=" + DEFAULTPRT; //0 or default : printer selcet,  1 : default print//기본프린터 프린터선택창x
				strAddData += "#AUTOPRINT=" + AUTOPRINT; //0 or default : manual print,  1 : auto print  //HIDEFRAME 사용할경우 출력(세트옵션)
				strAddData += "#VIRTUAL=" + VIRTUAL; //가상허용
				//strAddData += "#RENDEROPTION=" + RENDEROPTION; // Renderer Option  0 : Renderer 안씀  3 : Renderer 사용
				strAddData += "#PAPERSIZE=" + PAPERSIZE; // Paper Size  9 : A4  ,  1 : Letter   				
				//strAddData += "#ICP=" + ICP;
				strAddData += "#PRINTCNT=" + PRINTCNT;
				strAddData += "#AUTOCLOSE=" + AUTOCLOSE;
				strAddData += "#HIDECD=" + HIDECD;
				strAddData += "#PRTDLGOPT=" + PRTDLGOPT;		//0:default , 5:Partial print
				strAddData += "#CPFONTNAME=" + CPFONTNAME;
				strAddData += "#TRANSPARENCYIMG=" + TRANSPARENCYIMG;
				strAddData += "#SUPPORTPRTPORT=" + "UE9SVFBST01QVA=="; //1223 
				strAddData += "#_PRINTSTATUSDLG_=" + PRINTSTATUSDLG; //1223
				strAddData += "#SUPPORTPRTPORT=" + "Q1FNS1BSTjo=";
				strAddData += "#_PRINTCALLURLOPT_=" + PRINTCALLURLOPT;
				strAddData += "#_MASSPRINT_=1";
				if(CONVERTIMAGEPRINT.equals("1")){
					strAddData += "#_CONVERTIMAGEPRINT_=" + CONVERTIMAGEPRINT;
				}
				strAddData = replaceAll(strAddData, "\\n", "\n");

			}
			
            /*
             * [11단계] 메타파일 이름과 다운로드 URL을 서로 맞춥니다.
             *
             * 실제 저장 파일명:
             *   <JSESSIONID><today>.matmprp
             *
             * Viewer에 전달할 URL:
             *   Mafndown.jsp?fn=<today>
             *
             * Mafndown.jsp는 자신의 session.getId()를 앞에 붙여
             * 동일한 실제 파일명을 재조립합니다.
             */
			//File Server Setting
			String strSID = session.getId();
			String strEncFileServerIP = MaBase64Utils.base64Encode(strFileServerIp.getBytes("utf-8"));
			

            // 예: ABC123 + 20260730142000123 + .matmprp
			strMetaFilePath = strSID + today + ".matmprp";
			strDownFolder += "/" + strMetaFilePath; //저장해야 하는 경로에는 세션값을 넣고
			strDownURL += today; //보내야 할 URL에는 세션값이 빠진  날짜만 전송함
			
            /*
             * iUseNas == 1:
             * WAS/NAS 공유 경로에 Java FileWriter로 메타파일을 직접 저장합니다.
             *
             * iUseNas != 1:
             * masavefile.strSaveMetaFile()로 별도 파일 서버(FPSFM 계열)에 저장합니다.
             */
			if (iUseNas == 1) {
				BufferedWriter fw = null;
				try {
					fw = new BufferedWriter(new FileWriter(	new File(strDownFolder)));
					fw.write(strRetData + strAddData); //파일에다 작성
					fw.flush();
				} catch (IOException e) {
					System.out.println("markany_filewrite buffer Error");
					//e.printStackTrace(System.err);
				} finally {
					if (fw != null){
						fw.close(); //파일핸들 닫기
					}
				}

			} else {
				masavefile clMaSaveFile = new masavefile();
				strDownURL += "&fs=" + strEncFileServerIP;

				try {
					strRetCode = clMaSaveFile.strSaveMetaFile(
												strFileServerIp
												, iFileServerPort
												, strMetaFilePath
												, strRetData + strAddData
												, ""
												, "");

					iRetCode = Integer.parseInt(strRetCode);

				} catch (UnsatisfiedLinkError e) {
					System.out.println("markany_filewrite buffer Error");
					//e.printStackTrace(System.err);
				}

			} 
		
			String[] arrPDFName = strPdfFilePath.split(strIDECheck);
			String onlyPDF_FileName = arrPDFName[arrPDFName.length - 1];
			
			strPDFDownURL += onlyPDF_FileName.substring( 0, onlyPDF_FileName.lastIndexOf(".") );
			
			// ksk
			if(iUsePDF == 3) {
				strPDFDownURL += "#" + strOrgFilePath;
			}

            /*
             * Viewer 실행 후 다운로드 및 종료 정리에 사용할 값을 세션에 보관합니다.
             * Mafndown.jsp는 다운로드 완료 후 strRtDataFilePath와 strPdfFilePath를 삭제합니다.
             */
			session.setAttribute("strDownURL", strDownURL);
			session.setAttribute("strCookie", strCookie);
			session.setAttribute("strPdfFilePath", strPdfFilePath);
			session.setAttribute("strRtDataFilePath", strRtData);
		}
	}
	
    /*
     * [12단계] 서버에서 만든 URL을 브라우저 JavaScript와 로컬 Viewer에 안전하게 넘기기 위해
     * Base64 문자열로 변환합니다.
     */
	String 	strBase64DownURL = MaBase64Utils.base64Encode( strDownURL.getBytes ("utf-8") );
	String 	strBase64SessionURL = MaBase64Utils.base64Encode( strSessionURL.getBytes ("utf-8") );
	String 	strBase64PDFDownURL = MaBase64Utils.base64Encode( strPDFDownURL.getBytes ("utf-8") );
	String  strSPDFTotalURL = strPDFDownURL + "#" + strDownURL;
	String 	strBase64PDFTotalURL = MaBase64Utils.base64Encode( strSPDFTotalURL.getBytes ("utf-8") );
	session.setAttribute("strBase64PDFDownURL", strBase64PDFDownURL);
	session.setAttribute("strBase64PDFTotalURL", strBase64PDFTotalURL);
	
    /*
     * [13단계] 구형/대체 실행 경로에서 MaVerCheck.js의 LaunchRegistApp()에 넘길 명령을 결정합니다.
     *
     * quickurl : 빠른 실행 URL
     * registapp: 커스텀 URL 스킴 등록/실행
     * sockmeta : 이미 설치된 프로그램에 메타 URL 전달
     */
	String LaunchRegistAppCommand = "";
	if( iQuickSet == 1 ){
		LaunchRegistAppCommand = "quickurl";
	}else if( iQuickSet == 0 ){
		if( iSessionCheck == 0 ){
			LaunchRegistAppCommand = "registapp";
		}else{
			LaunchRegistAppCommand = "sockmeta";
		}
	}else if( iQuickSet == 2 ){
		LaunchRegistAppCommand = "sockmeta";
	}
	
	%>
	<!DOCTYPE HTML>
	<html>
	<head>
		<meta http-equiv="X-UA-Compatible" content="IE=edge" />
		<meta charset="utf-8">
		<TITLE>::: 전자확인증 발급 :::</TITLE>
		<link rel="stylesheet" type="text/css" href="<%=strWebHome%>/css/MaCommon.css">
		
		<!--
		<link rel="stylesheet" type="text/css" href="<%=strJsWebHome%>/installCheck/css/MaInstall.css">
		<script src="<%=strJsWebHome%>/installCheck/js/jquery-1.11.3.min.js" charset="utf-8"></script>
		<script src="<%=strJsWebHome%>/installCheck/js/jQuery.XDomainRequest.js" charset="utf-8"></script>
		<script src="<%=strJsWebHome%>/installCheck/js/MaInstallCheck_win.js?version=20170304" charset="utf-8"></script> 
		<script src="<%=strJsWebHome%>/MaVerCheck.js?version=20170402" charset="utf-8"></script> 		
		-->
		<!-- <script src="<%=strJsWebHome%>/jquery-1.12.1.min.js" charset="utf-8"></script> -->
		<script src="<%=strJsWebHome%>/jquery-3.7.1.min.js" charset="utf-8"></script>
		<!-- <script src="<%=strJsWebHome%>/jQuery.XDomainRequest.js" charset="utf-8"></script>	-->
        <!--
          [JS 연결 1]
          MaXHRControl.js는 브라우저와 로컬 MaEPSBroker.exe 사이의 통신 계층입니다.
          maBrokerInit(), craeteBoekerCommand(), maBroker_wss(), maBrokerController()가 정의됩니다.
          반드시 MaVerCheck.js보다 먼저 로드되어야 합니다.
        -->
		<script src="<%=strJsWebHome%>/MaXHRControl.js?version=20230320" charset="utf-8"></script> 		
        <!--
          [JS 연결 2]
          MaVerCheck.js는 설치 확인 UI와 실행 순서를 제어합니다.
          showPopup() -> maBrokerInit(getVersion) -> maFun_InstallCheck()
          -> maBrokerInit(executeBinary) -> maFun_execute() 순서입니다.
        -->
		<script src="<%=strJsWebHome%>/MaVerCheck.js?version=20180124" charset="utf-8"></script>
		<script>
            /*
             * [JS 연결 3] JSP의 Java 값을 브라우저 전역 JavaScript 변수로 변환합니다.
             *
             * vstrSCookie      : "Cookie: JSESSIONID=...;"의 Base64 값
             * vstrSDownURL     : Mafndown.jsp?fn=<today>[&fs=...]의 Base64 값
             * vstrSPDFDownURL  : PDF 다운로드 URL의 Base64 값
             * viUsePDF         : Viewer가 메타/PDF를 어떤 조합으로 받을지 결정
             * vstrApp          : mareportsafer 같은 커스텀 프로토콜/앱 식별자
             */
			var vstrSCookie				= "<%=strBase64Cookie%>";
			var vstrSDownURL			= "<%=strBase64DownURL%>";
			var vstrSessionCheck		= "<%=strSessionCheck%>";
			
			var vstrSPDFTotalURL		= "<%=strBase64PDFTotalURL%>";
			var vstrSPDFDownURL			= "<%=strBase64PDFDownURL%>";
			var vstrSSessionURL			= "<%=strBase64SessionURL%>";
			var viUsePDF				= "<%=iUsePDF%>";
			var viQuickSet				= "<%=iQuickSet%>";
			var vstrApp					= "<%=strApp%>"; 
			// ihkang
			// XHR 설치 체크 시 exe 다운 경로  
			var vstrInstallFilePath		= "<%=strDownFileUrl%>";
			// Common Version
			var iVersion				= "<%=strPVersion%>";	
	
            /*
             * [JS 연결 4] Broker 방식에서 사용할 두 배열을 JSP가 생성합니다.
             *
             * chkFileArray:
             *   MaXHRControl.js의 maBrokerInit(..., "getVersion", ...)가 읽어
             *   Broker에 설치 파일/버전 확인 명령을 만듭니다.
             *
             * executeBinaryArray:
             *   maBrokerInit(..., "executeBinary", ...)가 읽어
             *   ePageSaferRT.exe 실행 명령과 Viewer 파라미터를 만듭니다.
             */
			if (viQuickSet == 2 ){
				var checkFileInfo_1 		= [iVersion			, 0x0026	, "y85/zv65HGcOkFXjRS2WVg=="	, "TWFya0FueVxtYWVwc3J0"			];
				var chkFileArray 			= new Array(checkFileInfo_1);
				alert(checkFileInfo_1);
                /*
                 * GetParamData()는 MaVerCheck.js에 정의되어 있습니다.
                 * viUsePDF 값에 따라 urlmeta/urlpdf/prepdf 형식의 Viewer 실행 파라미터를 반환합니다.
                 */
				var executeBinaryInfo_1		= [GetParamData(vstrApp)		, 0x0026	, "y85/zv65HGcOkFXjRS2WVg=="	, "TWFya0FueVxtYWVwc3J0"		];
				var executeBinaryArray 		= new Array(executeBinaryInfo_1);
				alert(executeBinaryInfo_1);
				
			}else {
				
				var vstrIePopupURL			= "<%=strIePopupURL%>";
				var vpversion 				= "<%=pversion%>";
				var resize_height			= 800; //450 //1250
				var browserName				= <%=iSessionCheck%>===1?'installCheckSuccess':getBrowserName();
				switch (browserName){
					case "installCheckSuccess":
						resize_height = 500;
						break;
					case "Opera":
						resize_height = 600;
						break;
					case "Chrome":
						resize_height = 800; //950 //1250
						break;
					case "MSIE":
					case "Edge":
						resize_height = 800; //1250
						break;
					case "Firefox":
						resize_height = 650;
						break;
				}			
				window.resizeTo(300, resize_height); //550
			}
			window.name 				= 'popWinC';
		</script>
	</head>
<%
	if( iQuickSet == 2 ){ 
%>
	<BODY LEFTMARGIN="0" TOPMARGIN="0" RIGHTMARGIN="0" bottommargin="0" marginwidth="0" marginheight="0">
		<div id="popLayer" style="display: none;">
			<h3>ePageSAFER</h3>
			<div id="installSection">
				<div id="ment">증명서의 안전한 출력을 위해 보안 프로그램의 설치체크 여부를 확인 중입니다.</div>
				<div id="progressbar">
					<img src="<%=strImagePath%>/loading_c.gif">
				</div>
			</div>
			<div id="btnSection">
				<button id="btn_maInstall" class="btn-class" disabled onclick="javascript:location.href='<%=strUrlHome%>/bin/Setup_ePageSaferRT.exe'">다운로드</button>
				<button id="btn_closePopLayer" class="btn-class" onclick="javascript:closeWindow()">닫기</button>
				<section id="guideSection">
					<h3 class="guide-title">※ 설치 후 브라우저 설정 안내</h3>
					<p class="guide-desc">
						프로그램 설치 후 정상적으로 동작하지 않을 경우, 아래 안내에 따라 브라우저 설정을 확인해 주세요.
					</p>
					<div class="guide-step">
						<div class="step-header">
							<span class="step-number">1</span>
							<span class="step-text" id="step1Text"></span>
						</div>
						<div class="step-image">
							<img class="guide-img" data-guide="guide_site_setting" src="<%=strImagePath%>/guide_site_setting_chrome.png" alt="사이트 설정 진입 안내 이미지" style="max-width: 60%;">
						</div>
					</div>
					<div class="guide-step">
						<div class="step-header">
							<span class="step-number">2</span>
							<span class="step-text" id="step2Text"></span>
						</div>
						<div class="step-image">
							<img class="guide-img" data-guide="guide_app_allow" src="<%=strImagePath%>/guide_app_allow_chrome.png" alt="기기에 있는 앱 허용 설정 안내 이미지">
						</div>
					</div>
				</section>
				<p class="guide-notice">※ 위 안내 이미지는 이해를 돕기 위한 예시이며, 실제 화면과 다를 수 있습니다.</p>
				<style>
					#guideSection {
						max-width: 1200px;
						margin: 30px auto 0;
						padding: 28px 32px;
						border: 1px solid #d0d0d0;
						border-radius: 8px;
						background-color: #fafafa;
						text-align: center;
					}
					.guide-title { font-size: 17px; color: #333; margin: 0 0 8px 0; }
					.guide-desc { font-size: 15px; color: #d32f2f; font-weight: bold; margin: 0 0 24px 0; line-height: 1.6; }
					.guide-step { margin-bottom: 28px; text-align: center; }
					.guide-step:last-child { margin-bottom: 0; }
					.step-header { display: inline-flex; align-items: center; justify-content: center; margin-bottom: 14px; }
					.step-number { display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border-radius: 50%; background-color: #1a56db; color: #fff; font-size: 14px; font-weight: bold; margin-right: 10px; flex-shrink: 0; }
					.step-text { font-size: 16px; color: #333; line-height: 1.5; text-align: left; }
					.step-image { text-align: center; margin: 0 auto; }
					.step-image img { max-width: 70%; border: 1px solid #ddd; border-radius: 4px; box-shadow: 0 2px 6px rgba(0, 0, 0, 0.08); }
					.guide-notice { text-align: center; font-size: 13px; color: #999; margin-top: 16px; }
				</style>
				<script>
					// 브라우저별 안내 이미지/텍스트 분기
					// Edge: _edge / 그 외(Chrome / Whale 등): _chrome
					(function () {
						var IMAGE_BASE = "<%=strImagePath%>/";
						var ua = navigator.userAgent;

						// Edge만 분기 (Whale은 Chrome 기준으로 _chrome 사용)
						var isEdge = /\bEdg(e|A|iOS)?\//i.test(ua);  // 신/구버전 Edge
						var suffix = isEdge ? "_edge" : "_chrome";

						// 안내 이미지 교체
						var images = document.querySelectorAll(".guide-img");
						for (var i = 0; i < images.length; i++) {
							var name = images[i].getAttribute("data-guide");
							images[i].src = IMAGE_BASE + name + suffix + ".png";
						}

						// 안내 텍스트 교체 (브라우저별 설정 경로가 달라 문구도 분기)
						var guideText = {
							_chrome: {
								step1: '브라우저 주소창 옆 아이콘을 클릭한 후, <strong>사이트 설정</strong>을 선택합니다.',
								step2: '<strong>기기에 있는 앱</strong> 항목을 찾아 <strong>허용</strong>으로 변경합니다.'
							},
							_edge: {
								step1: '브라우저 주소창 옆 아이콘을 클릭한 후, <strong>이 사이트에 대한 사용 권한</strong>을 선택합니다.',
								step2: '<strong>사이트에서 로컬 네트워크의 모든 장치에 연결하도록 요청할 수 있습니다.</strong>를 <strong>허용</strong>으로 변경합니다.'
							}
						};

						var texts = guideText[suffix];
						var step1 = document.getElementById("step1Text");
						var step2 = document.getElementById("step2Text");
						if (step1) step1.innerHTML = texts.step1;
						if (step2) step2.innerHTML = texts.step2;
					})();
				</script>
			</div>
		</div>
	</BODY>
	</HTML>
	<script>
        /*
         * [JS 연결 5: Broker 실행 시작점]
         * 페이지 로드가 끝나면 MaVerCheck.js의 showPopup()을 호출합니다.
         *
         * showPopup()
         *   -> maBrokerInit(maFun_InstallCheck, "getVersion", true)
         *   -> MaXHRControl.js가 chkFileArray로 Broker 명령 생성
         *   -> 127.0.0.1:19877 WebSocket으로 MaEPSBroker.exe에 전송
         *   -> 응답이 정상이고 maOnlyInstallFlag == true
         *   -> maBrokerInit(maFun_execute, "executeBinary", false)
         *   -> executeBinaryArray를 이용해 ePageSaferRT.exe 실행
         */
		window.addEventListener('load', function() { showPopup(); }); //showPopup();
	</script>
<%
	}else {
%>
	<BODY LEFTMARGIN="0" TOPMARGIN="0" RIGHTMARGIN="0" bottommargin="0" marginwidth="0" marginheight="0">
		<iframe id="hidpopWinC" name="hidpopWinC" width=0 height=0 frameborder=0 marginheight=0 marginwidth=0 scrolling="auto"></iframe>
		<div class="container">
			<div class="content">
				<div class="section" id="browserChkImgSection">
					<img id="browserChkImg">
				</div>
			</div>
			<div class="section">
				<p id="browserChkComment"></p>
					
				<div class="section" id="printBtnSection">
					<input type="button" onclick="location.href='<%=strJspHome%>/MaSessionCheck.jsp'" value="문서발급">
					<br><br>
				</div>
				
			</div>
		</div>
		
		<script>
		var browserChkImg			= document.getElementById("browserChkImg");
		var browserChkComment		= document.getElementById("browserChkComment");
		var browserChkImgSection	= document.getElementById("browserChkImgSection");
		var printBtnSection			= document.getElementById("printBtnSection");
		
		if( viQuickSet == 2){
				// ksk
				var vstrParam = GetParamData(vstrApp);
				InstallCheck(1);
				browserChkImgSection.innerHTML ='<img src="<%=strImagePath%>/loading.gif" id="browserChkImg"><br>'
												+'<img src="<%=strImagePath%>/Ma_progressBar.gif"><br>'
												; //+'<img src="<%=strImagePath%>/notice.png"><br>';
				//browserChkComment.innerHTML =  browserChkComment.innerHTML + '<br><br> *미설치 혹은 업데이트가 필요한 경우 10초 후 수동설치 페이지로 이동합니다';
				printBtnSection.style.display = "none";
		}
		else {
            /*
             * iQuickSet != 2일 때 사용하는 커스텀 URL 스킴/세션 체크 경로입니다.
             * MaVerCheck.js의 LaunchRegistApp()이 mareportsafer:// 형식으로 Viewer를 호출합니다.
             */
			LaunchRegistApp(vstrApp, "<%=LaunchRegistAppCommand%>", vstrIePopupURL); 
			
			document.oncontextmenu		= document.body.oncontextmenu = function() {return false;}
		
			switch (browserName){
			case "installCheckSuccess":
				browserChkImgSection.innerHTML ='<img src="<%=strImagePath%>/loading.gif" id="browserChkImg"><br>'
												+'<img src="<%=strImagePath%>/Ma_progressBar.gif"><br>'
												+'<img src="<%=strImagePath%>/notice.png"><br>';
				printBtnSection.style.display = "none";
				break;
			case "Opera":
				browserChkImg.src = "<%=strImagePath%>/check_guide_opera.png";
				browserChkComment.innerHTML = 	'* 외부 프로토콜 요청 팝업이 보이실 경우 다음과 같이 진행해주십시오<br>'
												+ '<br> 1. [mareportsafer. 링크 항상 열기] 체크 합니다.'
												+ '<br> 2. [허용] 버튼을 누릅니다.';
				printBtnSection.style.display = "none";
				break;
			case "Chrome":
				browserChkImg.src = "<%=strImagePath%>/check_guide_chrome.png";
				browserChkComment.innerHTML = 	'* 외부 프로토콜 요청 팝업이 보이실 경우 다음과 같이 진행해주십시오<br>'
												+ '<br> 1. [위와같은 유형의 모든 링크에 대해 내 선택을 기억합니다.] 체크 합니다.'
												+ '<br> 2. [어플리케이션 시작] 버튼을 누릅니다.';
				printBtnSection.style.display = "none";
				break;
			case "MSIE":
			case "Edge":
				browserChkImgSection.innerHTML ='<img src="<%=strImagePath%>/loading.gif" id="browserChkImg"><br>'
												+'<img src="<%=strImagePath%>/Ma_progressBar.gif"><br>'
												+'<img src="<%=strImagePath%>/notice.png"><br>';
				printBtnSection.style.display = "none";
				browserChkComment.innerHTML = 	'<br>1. 미설치 및 PC에 설치된 버전이 낮을 경우 10초 후 설치페이지로 이동합니다.'
												+ '<br> 2. Microsoft Internet Explorer 8 이하 버전인 경우 팝업 항상 허용을 눌러주십시오.';
				break;
			case "Firefox":
				browserChkImg.src = "<%=strImagePath%>/check_guide_firefox.png";
				printBtnSection.style.display = "none";
				browserChkComment.innerHTML = 	'* 프로그램 실행 팝업이 보이실 경우 다음과 같이 진행해주십시오<br>'
												+ '<br> 1. [mareportsafer 링크에 대한 선택 사항을 기억 합니다.] 체크 합니다.'
												+ '<br> 2. [확인] 버튼을 누릅니다.';
												//+ '<br> 3. [문서발급] 버튼을 눌러 발급을 계속 진행합니다.';
				break;
			}
			
			if( browserName == 'Chrome' || browserName == 'Opera'){
				if( viQuickSet == "0"  ){
					browserChkComment.innerHTML =  browserChkComment.innerHTML + '<br> 3. [문서발급] 버튼을 눌러 발급을 계속 진행합니다.';
					printBtnSection.style.display = "block";
				}else {
					browserChkComment.innerHTML =  browserChkComment.innerHTML + '<br><br> *미설치 혹은 업데이트가 필요한 경우 10초 후 수동설치 페이지로 이동합니다';
				}				
			}
		}
		</script>
	</BODY>
	</HTML>
	
	
	<script>
		function resize(resize_height){
			alert(resize_height);
			window.resizeTo(1250, resize_height);
		}
		//resize(resize_height);
	</script>

<%
	}
%>


<%--
================================================================================
[이 파일 이후의 실행]
1. Broker가 ePageSaferRT.exe를 실행합니다.
2. Viewer는 executeBinaryArray/GetParamData()에서 받은 메타 다운로드 URL과
   Base64 Cookie(JSESSIONID)를 해석합니다.
3. Viewer가 Mafndown.jsp?fn=<today>[&fs=<Base64 파일서버IP>]를 HTTP 호출합니다.
4. Mafndown.jsp는 session.getId() + fn + ".matmprp"로 실제 파일명을 재조립합니다.
5. .matmprp를 Viewer에 바이너리 응답합니다.
6. Viewer가 메타데이터를 해석/복호화하고 PDF와 바코드를 렌더링합니다.
================================================================================
--%>
