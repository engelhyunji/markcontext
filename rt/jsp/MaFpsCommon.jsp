...
<%
	...
	*/
	...
	String  	strPVersion     					= "25314";
		
	//#############################		2D Bacode value set	###############################
	String		strMAServerIP 						= "127.0.0.1";
	//String		strMAServerIP 						= "192.168.1.11";
	// OZ## , 18200   / RP4# , 18320   / RD## , 18300
	int			iMAServerPort 						= 18200;
	String		strFunctionFlag 					= "";
	String		strSignCompany 						= "NONE";
	String		strReportCompany 					= "OZ##";	
	int			iCellBlockCount 				  	= 350;
	...
	String  	strCurrentPath 						= getServletContext().getRealPath("") + "/rt"; 
	//String  	strDownFolder       				= strCurrentPath + File.separator + "fn"; //metafile location	//log
	String  	strDownFolder       			= "/tomcat/apache-tomcat-10.1.25/webapps/ROOT/rt/Sample"; // 경로 설정
	...
	// ksk // 20180118 ihkang (사용하는 변수인지 확인 필요)
	...
	String		strPrintURL							= ""; // /ihkang/log/update.jsp
	String  	strPrintParam						= ""; // ?no_appeal=20220721test
	...
	String		RENDEROPTION						= "2";  					//Renderer Option  0 : Renderer 안씀  3 : Renderer 사용(기본)
	String		PRTTYPE		          				= "0"; 						// 출력을 위한 옵션 - 0 : 기본 출력[Debunu], 1 : PDF --> BMP로 변환해서 출력[BMP 출력], 2 : PDF -> tiff 변환 --> PDF 변환 후 출력[Debenu]
	String		PRTDLGOPT					  		= "5"; 						// 다이얼로그 이미지 뺀거로 변경 // 이미지 사용O : 0(default), 이미지 사용X : 1, 이미지 사용X/CloseX : 2, 부분출력 : 5
	...
	String		PRINTCOPIES							= "2";  					// 인쇄 한번에 출력되는 수  
	String		MAXPRTCOPIES						= "10";    					// 사용자가 선택할 수 있는 최대 출력 매수( 사용하려면 2 이상이어야 사용할 수 있음 )
	String   	PRINTSTATUSDLG = "1";  //1223
    String  	SUPPORTPRTPORT ="UE9SVFBST01QVA=="; //1223
	out.println(PRINTCOPIES);
	//#############################		RD Reporting value	###############################		
	//Pattern Image...
	...
	//2D바코드x좌표^2D바코드 y좌표^복사방지 x좌표^복사방지 y좌표^
	strMAConfigData	 							= "11^200^6^250^528^264^internet^copy^4^280^82^0^0^3^";		//11	// 가로"31^250^6^250^528^264^internet^copy^4^280^82^0^0^3^";
