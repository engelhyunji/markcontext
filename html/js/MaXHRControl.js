...
function maBroker_wss(bokerCommand){
  ...
  ws.onopen = function() {
    MDBG("onopen-websock", "called");
    ws.send(brokerParam);
  };
  ws.onmessage = function(event) {
	MDBG("onmessage-websock", "called");
    console.log("onmessage", event);
    //eval(event.data);
	let eventData = event.data;
    eventData = eventData.replace('maCbBrokerResFun(','');
    eventData = eventData.replace(')','');
    let eventDataJson = JSON.parse(eventData);
    maCbBrokerResFun(eventDataJson);
	
    ws.close();
  };
  ws.onerror = function(e) {
	MDBG("onerror-websock", "called");
    maBrokerController(null, null, e);
	ws.close();
    reConnectionMaBroker(maTimeOutCheck_s); //modify
  };
  ws.onclose = function() {
    MDBG("onclose", "called");
  };
...
function reConnectionMaBroker(reConnectionTime_s) {
  setTimeout(function () {
	MDBG("reConnectionMaBroker", reConnectionTime_s);
    if( window.WebSocket ){
...
function maSetBrokerErr(err) {
  switch (err) {
    ...
    case -1000:
      maStrError = "RETCODE_NOTFOUND_FILE";
      break;
	
	case -2001:
      maStrError = "RETCODE_REQDATA_ZEROSIZE";
      break;

    case -2002:
      maStrError = "RETCODE_REQDATA_NOTFOUND_SIGN";
      break;

    case -2003:
      maStrError = "RETCODE_REQDATA_JSONPARSER_ERR";
      break;
	  
	case -3000:
      maStrError = "RETCODE_PRTDAT_DOWNLOAD_FAIL";
      break;

    case -3001:
      maStrError = "RETCODE_PRTDAT_HASH_MISMATCH";
      break;

    case -3002:
      maStrError = "RETCODE_PRTDAT_URL_NOT_FOUND";
      break;

    case -4001:
      maStrError = "RETCODE_EXECUTE_FILENAME_ERR";
      break;
	  
    case -4002:
      maStrError = "RETCODE_EXECUTE_NOT_FOUND_MARKANY_SUBJECT";
      break;

    case -4003:
      maStrError = "RETCODE_EXECUTE_CODESIGN_CHECK_ERR";
      break;

    case -4004:
      maStrError = "RETCODE_EXECUTE_DATA_VALIDATION_FAIL";
      break;

    case -4005:
      maStrError = "RETCODE_EXECUTE_PARAM_VALIDATION_FAIL";
      break;
	  
	case -5000:
      maStrError = "RETCODE_PRTINFO_DEFAULT";
      break; 

	case -5001:
      maStrError = "RETCODE_PRTINFO_DATFILE_NOTFOUND";
      break; 
	  
	case -5002:
      maStrError = "RETCODE_PRTINFO_PRTCHECK_DLL_NOTFOUND";
      break; 
