...
function makeMetaURL() {
	var metaUrl = ""; 
	if(viUseEncCookie == 0){
		if(vstrIEBrowserE.length === 2){
			metaUrl = vstrSDownURL + "@" + vstrSCookie + "@" + base64_encode(getAdams()) + "@IEL" + vstrIEBrowserE;
		}else{
			metaUrl = vstrSDownURL + "@" + vstrSCookie + "@" + base64_encode(getAdams());
		}
		return metaUrl;
	}else if(viUseEncCookie == 1){
		metaUrl = base64_encode(vstrSDownURL + ">" + vstrSCookie + ">" + base64_encode(getAdams())) 
		return metaUrl;
	}
	/*
...
var langObj_Kor = {
		langType 			: 'kor',
		installCheck 		: '증명서의 안전한 출력을 위해 보안 프로그램의 설치체크 여부를 확인 중입니다.',
		installComplete		: '설치체크가 완료되었습니다.<br> 증명서 미리보기 화면과 출력이 가능한 뷰어가 실행되며 잠시 후 팝업창이 자동으로 종료됩니다.',
		installNeed			: '위변조 방지 솔루션의 설치 혹은 업데이트가 필요합니다.<br> 아래의 다운로드 버튼을 눌러 다운로드 후 설치를 진행하세요.<br>',
		viewerLoadFail		: '뷰어 실행에 실패하였습니다.<br> 관리자에게 문의해주시기 바랍니다.'
};
var langObj_En = {
		langType 			: 'en',
		installCheck 		: 'installChecking',
		installComplete		: 'installComplete. load Viewer',
		installNeed			: 'install Need',
		viewerLoadFail		: 'viewerload fail'
};
var langObjArray = new Array(null, langObj_Kor, langObj_En);

showPopup = function() {
	var popUp = $("#popLayer");
	popUp.show();
	popUp.css("position", "absolute");
	popUp.css("top", "10px"); //88px
	popUp.css("left", "54px");
	$("body").css("background-color", "#565656");
	$("#ment").html(langObjArray[langType].installCheck);
...
function maFun_InstallCheck() {
	...
	if (maOnlyInstallFlag == true) {
		...
		$("#ment").html(langObjArray[langType].installNeed);
	}
}
	...
	if (typeof (maResJsonData) != 'string') {
		...
		for (var execute_i = 0; execute_i < executeBinaryObj.length; execute_i++) {
			...
			if (executeFalg == "true") {
				...
				$("#ment").html(langObjArray[langType].installComplete);
				setTimeout(function(){
					closePopLayer();
					closeWindow();
				}, 3000);
				...
				$("#ment").html(langObjArray[langType].viewerLoadFail);
			}
		}
	}
}
