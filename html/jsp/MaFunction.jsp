...
<%!
	...
	public boolean deleteFile(String filePath) {
		...
		try {
			...
			System.out.println("## deleteFile() NullPointerException " + filePath);
			e.printStackTrace();
		} catch (Exception e) {
			System.out.println("## Ma File Delete Fail... " + filePath);
			e.printStackTrace();
		} finally {
	...
	public String Hash(String str, String hashtype) {
		...
		try {
			...
			System.out.println("## Hash() IndexOutOfBoundsException" + e.toString());
		} catch (Exception e) {
			SHA = e.toString();
		}
		return SHA;
	...
	//************************* Multi OS ***************************//
	//20160713_hcchoi - addZeroPadding
	public String zeroPadding(String str, int paddingSize){
		byte[] arrNumber = str.getBytes();
		byte[] arrPaddingNumber = new byte[paddingSize];
		int zeroCount = arrPaddingNumber.length - arrNumber.length;

		for(int i=0; i<paddingSize; i++){
			if(zeroCount > i){
				arrPaddingNumber[i] = '0';
			}else{
				arrPaddingNumber[i] = arrNumber[i - zeroCount];
			}
		}
		return new String(arrPaddingNumber);
	}	
%>
